...
import proyectorutas.accesoadatos.UsuarioDAL;
import proyectorutas.accesoadatos.RolDAL;
import proyectorutas.appweb.utils.*;
import proyectorutas.en.Rol;
import proyectorutas.en.Usuario;
        ...
        try
        {
            ...
            if(usuario_result.getId() > 0)
            {
                ...
                request.getRequestDispatcher(
                        "Views/Usuario/cambiarPassword.jsp")
                    .forward(request, response);
            }
            else
            {
                Utilidad.enviarError("El login: " + usuario_result.getLogin() +
                        "no existe en la Base de Datos", request, response);
            }
        }
        catch(Exception ex)
